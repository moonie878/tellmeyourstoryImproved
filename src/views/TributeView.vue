    ...
    <section class="builder-section">
      <div class="builder-inner">
        ...
        <div v-if="currentStep === 0" class="builder-card">
          ...
          <div class="field-group">
            ...
            <input
              v-model="form.name"
              type="text"
              placeholder="e.g. Margaret Eleanor Griffiths"
              class="field-input"
              maxlength="80"
            />
          </div>
          ...
          <div class="field-group">
            ...
            <textarea
              ...
              rows="4"
              placeholder="A short tribute that will appear as a beautiful quote slide in the video…"
        ...
        <div v-else-if="currentStep === 1" class="builder-card">
          ...
          <p class="card-sub">
            Upload up to 30 photos. Drag to reorder them — the first photo
            will be used as the opening frame.
          </p>
          ...
          >
            ...
            <div class="upload-zone-content">
              ...
              <p class="upload-text">
                Drop photos here or <span class="upload-link">browse files</span>
              </p>
          ...
          <p v-if="form.photos.length === 0" class="photo-empty">
            No photos added yet — upload at least one to continue.
          </p>
          ...
          <div class="field-group">
            ...
            <div class="duration-row">
              <button
                ...
                :key="d.value"
                @click="form.slideDuration = d.value as TributeSlideDuration"
                class="duration-btn"
                :class="form.slideDuration === d.value ? 'duration-active' : 'duration-idle'"
              >
          ...
          <div class="field-group">
            ...
            <div class="transition-row">
              <button
                ...
                :key="t.value"
                @click="form.transition = t.value as TributeTransition"
                class="transition-btn"
                :class="form.transition === t.value ? 'transition-active' : 'transition-idle'"
              >
                <!-- Mini preview -->
                <div class="transition-preview">
                  <div v-if="t.value === 'cut'" class="t-cut">
                    <div class="t-half-l"></div>
                    <div class="t-half-r"></div>
                  </div>
        ...
        <div v-else-if="currentStep === 3" class="builder-card">
          ...
          <div class="summary-card">
            ...
            <div v-if="form.birthYear || form.deathYear" class="summary-row">
              ...
              <span class="summary-value">
                {{ [form.birthYear, form.deathYear].filter(Boolean).join(' — ') }}
              </span>
            </div>
            <div class="summary-row">
              <span class="summary-label">Photos</span>
              <span class="summary-value">{{ form.photos.length }} photos</span>
            </div>
            <div class="summary-row">
              <span class="summary-label">Music</span>
              <span class="summary-value">{{ MUSIC_TRACKS[form.musicTrack].label }}</span>
            </div>
            <div class="summary-row">
              <span class="summary-label">Duration</span>
              <span class="summary-value">~{{ estimatedLength }}</span>
            </div>
          </div>
          ...
          <!-- Action buttons -->
          <div v-if="!isGenerating" class="export-buttons">
            <!-- Free preview -->
            <button
              @click="handlePreview"
              class="btn-preview"
  ...
  <div>
    ...
    <p class="btn-sub-label">
      {{ isTier4 ? 'Included in your Full Collection plan ✦' : 'No watermark · Full HD · Keep forever' }}
          ...
          <button @click="currentStep--" class="btn-back-link">
            ← Edit your tribute
          </button>
        </div>
      ...
      <div class="why-inner">
        ...
        <div class="why-grid">
          <div class="why-card">
            ...
            <p class="why-card-title">Up to 30 photos</p>
            <p class="why-card-desc">
              Every photo flows beautifully with elegant transitions and
              a cinematic style.
            </p>
          </div>
          <div class="why-card">
            <span class="why-icon">🎵</span>
            <p class="why-card-title">Music that fits</p>
            <p class="why-card-desc">
              Choose from curated gentle tracks or upload your own music
              that was meaningful to them.
            </p>
          </div>
          <div class="why-card">
            <span class="why-icon">💛</span>
            <p class="why-card-title">Their words</p>
            <p class="why-card-desc">
              Add a tribute that becomes a beautiful quote slide woven
              through the video.
            </p>
          </div>
          <div class="why-card">
            <span class="why-icon">🎬</span>
            <p class="why-card-title">Full HD MP4</p>
            <p class="why-card-desc">
              Download and share with family, play at a memorial service,
              or keep it forever.
            </p>
          </div>
        </div>
        ...
        <div class="upsell-card">
          ...
          <h3 class="upsell-title">
            Capture their full life story with Tell Me Your Story
          </h3>
          <p class="upsell-desc">
            100 guided questions across 10 chapters. Every answer turns
            into a beautifully designed keepsake book — a complete record
            of their life, in their own words.
          </p>
          <router-link to="/register" class="upsell-btn">
...
const estimatedLength = computed(() => {
  const slides = form.value.photos.length + 2 // photos + title + closing
    + (form.value.tribute.trim() ? 1 : 0) // tribute text slide
  const transitionSecs = form.value.transition === 'cut' ? 0
    : form.value.transition === 'fade' ? 1 : 2
  const totalSecs = slides * form.value.slideDuration
    + (slides - 1) * transitionSecs
...
async function addFiles(files: File[]) {
  const remaining = 30 - form.value.photos.length
...
.tribute-hero::before {
  ...
  position: absolute;
  inset: 0;
  background: radial-gradient(ellipse at 50% 0%, rgba(148,116,74,0.15) 0%, transparent 70%);
  pointer-events: none;
}
...
.photo-btn:hover { background: white; }
...
.music-upload-btn:hover { border-color: #7C5C3B; }
...
.t-fade {
  height: 100%;
  background: linear-gradient(to right, #C4B8AC, #7C5C3B);
}

.t-slow-fade {
  height: 100%;
  background: linear-gradient(to right, #C4B8AC 20%, #A89880, #7C5C3B 80%);
}
