...
import { isValidStoryType, savePendingStoryType } from '../lib/pendingStoryTypes'
...
// Story type picked on the homepage ("Whose story?"), e.g. ?type=mum
const storyType = computed(() =>
  isValidStoryType(route.query.type) ? (route.query.type as string) : undefined,
)
watch(storyType, (type) => { if (type) savePendingStoryType(type) }, { immediate: true })
...
const loginRoute = computed(() => {
  ...
  if (storyType.value) params.push(`type=${storyType.value}`)
...
function getRedirectPath() {
  ...
  if (planKey.value) params.set('plan', planKey.value)
  if (storyType.value) params.set('type', storyType.value)
